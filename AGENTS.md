...
- 2026-05-19: **修复 Lienol Full 编译错误目标（mediatek/filogic 而非 ramips/mt7621）**：根因：`config_for_Lienol` 中 `CONFIG_LINUX_6_6=y`，但 Lienol 25.12 的 ramips/mt7621 内核版本是 6.12（`KERNEL_PATCHVER:=6.12`），6.6 内核选项不存在，`make defconfig` 重置目标为默认的 mediatek/filogic（也是 6.12）。修复：(1) `CONFIG_LINUX_6_6=y` → `CONFIG_LINUX_6_12=y`；(2) `validate_build_output.py` 新增 `EXPECTED_TARGET_DIR` 环境变量校验，防止非目标架构固件通过验证；(3) 工作流设置 `EXPECTED_TARGET_DIR: ramips/mt7621`。
- 2026-05-20: **修复 Lienol Full 配置文件未被加载**：`Load custom configuration` 步骤 `cd /workdir` 后用相对路径 `$CONFIG_FILE`（`custom_configs/config_for_Lienol`）查找配置文件，但该文件在 checkout 目录（`$GITHUB_WORKSPACE`）下而非 `/workdir`，导致 `[ -e $CONFIG_FILE ]` 返回 false，`.config` 从未被复制，OpenWrt 使用默认配置编译出 mediatek/filogic。修复：`$CONFIG_FILE` → `$GITHUB_WORKSPACE/$CONFIG_FILE`，`files` → `$GITHUB_WORKSPACE/files`。
