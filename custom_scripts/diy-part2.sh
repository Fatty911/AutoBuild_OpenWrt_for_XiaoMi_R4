...
if [ -f "$GITHUB_WORKSPACE/custom_scripts/fix_dts_nvmem_layout.py" ]; then
  python3 "$GITHUB_WORKSPACE/custom_scripts/fix_dts_nvmem_layout.py" .
else
  echo "fix_dts_nvmem_layout.py 不存在，跳过 DTS 修复"
fi
