...
def main():
    ...
    providers_to_try = providers[:MAX_PROVIDER_TRIES]
    print(f"并行尝试 {len(providers_to_try)} 个提供商: {[p['name'] for p in providers_to_try]}")
    ...
    def call_provider_wrapper(provider):
        """包装函数用于并行调用"""
        result = try_provider(
            provider["name"],
            provider["proxy_url"],
    ...
    with ThreadPoolExecutor(max_workers=len(providers_to_try)) as executor:
        futures = {executor.submit(call_provider_wrapper, p): p for p in providers_to_try}
        for future in as_completed(futures):
            provider_name, result = future.result()
            if result:
                fixed_content = result
                used_provider = provider_name
                print(f"[{provider_name}] ✅ 成功获取修复结果，取消其他并行任务")
                # 取消其他还在运行的任务
                for f in futures:
                    if f != future:
                        f.cancel()
                ...
                print(f"[{provider_name}] ❌ 失败，继续等待其他提供商...")

    # 如果并行都失败，再串行尝试剩余的 provider（最多再试2个）
    if not fixed_content and len(providers) > MAX_PROVIDER_TRIES:
        remaining = providers[MAX_PROVIDER_TRIES:MAX_PROVIDER_TRIES+2]
        print(f"并行调用失败，串行尝试剩余 {len(remaining)} 个提供商...")
